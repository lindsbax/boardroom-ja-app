...
function GoldButton({ children, onClick, href, disabled, type = 'button' }) {
...
const TABS = [
  { key: 'directory', label: 'Directory', icon: Building2, Comp: DirectoryTab },
  { key: 'events', label: 'Events', icon: Calendar, Comp: EventsTab },
  { key: 'skillswap', label: 'Skillswap', icon: Users, Comp: SkillswapTab },
  { key: 'shop', label: 'Shop', icon: ShoppingBag, Comp: ShopTab },
  { key: 'jobs', label: 'Jobs', icon: Briefcase, Comp: JobsTab },
  { key: 'resources', label: 'Resources', icon: FileText, Comp: ResourcesTab },
  { key: 'messages', label: 'Messages', icon: Inbox, Comp: MessagesTab },
];
  ...
  return (
    <div className="min-h-screen bg-ivory font-sans">
            ...
            {TABS.map((t) => (
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-3 text-[13px] tracking-wide uppercase whitespace-nowrap border-b-2 transition-colors ${
                  tab === t.key ? 'border-gold text-gold' : 'border-transparent text-ivory/60 hover:text-ivory'
                }`}
              >
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        <div className="mb-6">
          <h1 className="font-display text-2xl text-navy">{active.label}</h1>
        </div>
